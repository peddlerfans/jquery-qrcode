...
useMutationObserver(
  document.head,
  (mutations:any) => {
    const hasCustomStyleEl = mutations.some((n:any) => Array.from(n.addedNodes).includes(styleDom));
    if (!hasCustomStyleEl) {
      document.head.appendChild(styleDom);
...
primaryColor &&
  ...
  //   parameters += `${key}=${encodeURIComponent(obj[str as keyof typeof obj])}&`;
  ...
  actions: {
    ...
    setTheme(theme: Partial<ThemeState>) {
      ...
      for (const key in theme) {
        // this[key] = theme[str as keyof typeof theme]
        // this[key] = theme[key];
      }
