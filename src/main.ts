...
import {Form,Input,InputNumber,Button,Drawer,Switch,Slider,Select,SelectOption} from 'ant-design-vue';
 
mockEnv.includes(import.meta.env.MODE as EnvType) && mock === 'on' && enableMock()

createApp(App).use(Form).use(Input).use(InputNumber).use(Button).use(Switch).use(SelectOption).use(Select).use(Slider).use(Drawer).use(createPinia()).use(router).mount('#app')
