	...
	var FreeTransform = core_mjs.mvc.View.extend({
	  ...
	  startListening: function startListening() {
	    ...
	    var _this$options = this.options,
	      cell = _this$options.cell,
	      paper = _this$options.paper,
	      graph = _this$options.graph,
	      clearOnBlankPointerdown = _this$options.clearOnBlankPointerdown;
	    ...
	    this.listenTo(paper, 'scale translate', function () {
	      return _this.requestUpdate();
	    });

	    // Remove the freeTransform when the model is removed.
	    this.listenTo(graph, 'reset', function () {
	      return _this.remove();
	    });
	    this.listenTo(cell, 'remove', function () {
	      return _this.remove();
	    });
	    if (clearOnBlankPointerdown) {
	      // Hide the freeTransform when the user clicks anywhere in the paper
	      this.listenTo(paper, 'blank:pointerdown', function () {
	        return _this.remove();
	      });
	    }
	  },
	  onCellAttributeChange: function onCellAttributeChange(_cell, _changed, opt) {
	  ...
	  render: function render() {
	    ...
	    var cell = options.cell,
	      ...
	      allowOrthogonalResize = options.allowOrthogonalResize;
	  ...
	  requestUpdate: function requestUpdate(opt) {
	    var UPDATE_PRIORITY = this.UPDATE_PRIORITY,
	  ...
	  updateFrameScaled: function updateFrameScaled() {
	    var _this$options2 = this.options,
	      paper = _this$options2.paper,
	      cell = _this$options2.cell,
	    ...
	    var _cell$getBBox$moveAnd = cell.getBBox().moveAndExpand({
	        x: -left,
	        y: -top,
	        width: left + right,
	        height: top + bottom
	      }),
	      x = _cell$getBBox$moveAnd.x,
	      y = _cell$getBBox$moveAnd.y,
	      width = _cell$getBBox$moveAnd.width,
	      height = _cell$getBBox$moveAnd.height;
	    var angle = cell.angle();
	    var transformMatrix = paper.matrix() // transform-origin at 0 0
	    .translate(x, y).translate(width / 2, height / 2) // move transform-origin at 50% 50%
	    .rotate(angle).translate(-width / 2, -height / 2);
	    var transformString = core_mjs.V.matrixToTransformString(transformMatrix);
	    this.$el.css({
	      'width': width,
	      'height': height,
	      'left': 0,
	  ...
	  updateFrameNoScale: function updateFrameNoScale() {
	    var _this$options3 = this.options,
	      paper = _this$options3.paper,
	      cell = _this$options3.cell,
	      padding = _this$options3.padding;
	    var _util$normalizeSides2 = core_mjs.util.normalizeSides(padding),
	      left = _util$normalizeSides2.left,
	      right = _util$normalizeSides2.right,
	      top = _util$normalizeSides2.top,
	      bottom = _util$normalizeSides2.bottom;
	    ...
	    var transformString = "rotate(".concat(angle, "deg)");
	    this.$el.css({
	      'width': bbox.width,
	      'height': bbox.height,
	      'left': bbox.x,
	  ...
	  updateHandleDirections: function updateHandleDirections() {
	  ...
	  calculateTrueDirection: function calculateTrueDirection(relativeDirection) {
	  ...
	  startResizing: function startResizing(evt) {
	    ...
	    var calcMinWidth = typeof minWidth === 'function' ? minWidth.call(this, cell, this) : minWidth;
	    var calcMinHeight = typeof minHeight === 'function' ? minHeight.call(this, cell, this) : minHeight;
	    var calcMaxWidth = typeof maxWidth === 'function' ? maxWidth.call(this, cell, this) : maxWidth;
	    var calcMaxHeight = typeof maxHeight === 'function' ? maxHeight.call(this, cell, this) : maxHeight;
	    ...
	    this.trigger("".concat(data.action, ":start"), evt);
	  },
	  toValidResizeDirection: function toValidResizeDirection(direction) {
	    return {
	      'top': 'top-left',
	      'bottom': 'bottom-right',
	      'left': 'bottom-left',
	  ...
	  pointermove: function pointermove(evt) {
	    ...
	    switch (action) {
	      case 'resize':
	        ...
	        // Fitting into a grid
	        width = core_mjs.g.snapToGrid(width, horizontalGrid);
	        height = core_mjs.g.snapToGrid(height, verticalGrid);
	        // Minimum
	      ...
	      case 'rotate':
	        ...
	        model.rotate(core_mjs.g.snapToGrid(i.modelAngle + theta, options.rotateAngleGrid), true, null, {
	          freeTransform: this.cid
	        });
	        break;
	    }
	    this.trigger("".concat(action), evt);
	  },
	  pointerup: function pointerup(evt) {
	    ...
	    this.stopOp();
	    this.trigger("".concat(data.action, ":stop"), evt);
	  ...
	  removeInstancesForPaper: function removeInstancesForPaper(paper) {
	    core_mjs.util.invoke(this.getInstancesForPaper(paper), 'remove');
	  },
	  getInstancesForPaper: function getInstancesForPaper(paper) {
	    return this.instancesByPaper[paper.cid] || {};
	  },
	  registerInstanceToPaper: function registerInstanceToPaper(instance, paper) {
	    this.instancesByPaper[paper.cid] || (this.instancesByPaper[paper.cid] = {});
	    this.instancesByPaper[paper.cid][instance.cid] = instance;
	  },
	  unregisterInstanceFromPaper: function unregisterInstanceFromPaper(instance, paper) {
