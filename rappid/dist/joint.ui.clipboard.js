	...
	*/
	...
	var Clipboard = Backbone.Collection.extend({
	  ...
	  constructor: function constructor(options) {
	    ...
	    this.defaults = core_mjs.util.assign({}, this.defaults, options);
	    this.cid = core_mjs.util.guid();
	  },
	  /**
	   * @public
	   ...
	   */
	  copyElements: function copyElements(selection, graph, opt) {
	    opt = core_mjs.util.assign({}, this.defaults, opt);
	    var originalElements = Array.isArray(selection) ? selection : selection.toArray();
	    var cells = core_mjs.util.sortBy(Object.values(graph.cloneSubgraph(originalElements, opt)), function (cell) {
	   ...
	   */
	  cutElements: function cutElements(selection, graph, opt) {
	    opt = core_mjs.util.assign({}, this.defaults, opt);
	    var elementsToRemove = this.copyElements(selection, graph, opt);
	    var removeCellOptions = opt.removeCellOptions || {};
	    removeCellOptions.clipboard = this.cid;
	    graph.startBatch('cut');
	    core_mjs.util.invoke(elementsToRemove, 'remove', removeCellOptions);
	    graph.stopBatch('cut');
	   ...
	   * @returns {Array.<dia.Cell>}
	   */
	  pasteCells: function pasteCells(graph, opt) {
	    var _this = this;
	    opt = core_mjs.util.assign({}, this.defaults, opt);
	    if (opt.useLocalStorage) {
	      this.updateFromStorage(graph);
	    }
	    var currentZIndex = graph.maxZIndex();
	    //process modification on current data
	    var modifiedCells = this.map(function (cell) {
	      currentZIndex += 1;
	      return _this.modifyCell(cell, opt, currentZIndex);
	    });
	    var addCellOptions = opt.addCellOptions || {};
	    addCellOptions.clipboard = this.cid;
	    var pastedCells = core_mjs.util.sortBy(modifiedCells, function (cell) {
	   ...
	   * @returns {Array.<dia.Cell>}
	   */
	  pasteCellsAtPoint: function pasteCellsAtPoint(graph, point, opt) {
	    var _this2 = this;
	    opt = core_mjs.util.assign({}, this.defaults, opt);
	    if (opt.useLocalStorage) {
	      this.updateFromStorage(graph);
	    ...
	    var clones = core_mjs.util.sortBy(Object.values(graph.cloneSubgraph(this.toArray(), opt)), function (cell) {
	      return cell.attributes.z || 0;
	    });

	    // Creating temporary graph to properly calculate bbox of links
	    // Using clones to not mess up graph references
	    var cellNamespace = graph.get('cells').cellNamespace;
	    var tmpGraph = new core_mjs.dia.Graph([], {
	      cellNamespace: cellNamespace
	    });
	    tmpGraph.resetCells(clones, {
	      sort: false
	    });
	    var originPoint = this.getOriginPoint(clones, tmpGraph, opt.origin);
	    ...
	    var modifiedCells = this.map(function (cell) {
	      currentZIndex += 1;
	      return _this2.modifyCell(cell, opt, currentZIndex);
	    });
	    var addCellOptions = opt.addCellOptions || {};
	    addCellOptions.clipboard = this.cid;
	    var pastedCells = core_mjs.util.sortBy(modifiedCells, function (cell) {
	  ...
	  isEmpty: function isEmpty(opt) {
	    ...
	    if (opt.useLocalStorage) {
	      var cells = this.getJSONFromStorage();
	   ...
	   * @returns {dia.Cell}
	   */
	  modifyCell: function modifyCell(cell, opt, z) {
	    cell.set('z', z);
	    if (cell.isLink() && opt.link) {
	      cell.set(opt.link);
	    }
	    if (opt.translate) {
	      var _opt$translate = opt.translate,
	        dx = _opt$translate.dx,
	        dy = _opt$translate.dy;
	   ...
	   * @param {dia.Graph} graph
	   */
	  updateFromStorage: function updateFromStorage(graph) {
	    var cells = this.getJSONFromStorage();
	    if (!cells) return;
	    var graphJSON = {
	      cells: cells
	   ...
	   */
	  getJSONFromStorage: function getJSONFromStorage() {
	   ...
	   */
	  getOriginPoint: function getOriginPoint(cells, graph, origin) {
	    var cellsBBox = graph.getCellsBBox(cells);
	    if (cellsBBox) {
	      return core_mjs.util.getRectPoint(cellsBBox, origin);
	    } else {
	      return null;
	    }
	  }
