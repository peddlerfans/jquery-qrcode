...
(function (exports, $, core_mjs) {
	...
	*/
	...
	function print(paper, opt) {
	  ...
	  var printAction = function printAction(pages) {
	 ...
	 */
	var preparePrintArea = function preparePrintArea(paper, areaToPrintBBox, opt) {
	  ...
	  var printSVGVel = core_mjs.V(paper.svg).clone();
	  var printLayersVel = printSVGVel.findOne(".".concat(core_mjs.util.addClassNamePrefix('layers')));
	  ...
	  var _getViewBox = getViewBox(paperBBox, areaToPrintBBox, sheetSizePx),
	    scaleToFit = _getViewBox.scaleToFit,
	    bBox = _getViewBox.bBox;
	  $printPaper.css({
	    left: 0,
	    top: 0
	  });
	  ...
	  $printArea.append([$printPaper.append([printSVGVel.node])]);
	...
	var sendToPrinter = function sendToPrinter(paper, pages, opt) {
	  if (pages) {
	    ...
	    */
	    ...
	    pages.forEach(function ($page) {
	      $page.removeClass('preview').addClass('print-ready').appendTo($body);
	    });
	    var called = false;
	    var onceAfterPrint = function onceAfterPrint() {
	      if (!called) {
	        called = true;
	...
	var preparePages = function preparePages(paper, options) {
	...
	var getSheetSizePx = function getSheetSizePx(opt) {
	...
	var getViewBox = function getViewBox(paperAreaBBox, areaToPrint, sheetSizePx) {
	...
	var resolveOptions = function resolveOptions(opt) {
	  var options = core_mjs.util.defaultsDeep({}, opt, {
	    ...
	    sheetUnit: 'mm',
	    ready: function ready(printAreaElements, readyToPrint, opt) {
	...
	var injectCss = function injectCss(cssString) {
	...
	var injectPrintCss = function injectPrintCss(printAreaObjCss, opt) {
	...
	var getArea = function getArea(paper, opt) {
	  var area = opt.area;
	...
	var splitArea = function splitArea(area, chunkSize) {
	...
	var getPosterPieceSize = function getPosterPieceSize(area, opt) {
	  var pieceSize = {
	    width: opt.width,
	    height: opt.height
	  };
	...
	var convert = {
	  supportedUnits: {
	    ...
	    'pc': function pc(value) {
	      return this.millimeterSize * (value * 25.4 / 6);
	    }
	  },
	  measure: function measure(width, height, unit) {
	    unit = unit || '';
	    var box = $('<div/>').css({
