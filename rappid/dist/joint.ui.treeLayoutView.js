...
(function (exports, $, core_mjs) {
  ...
  */
  ...
  var TreeLayoutView = core_mjs.mvc.View.extend({
    ...
    toggleDefaultInteraction: function toggleDefaultInteraction(interactive) {
      this.options.paper.setInteractivity(interactive);
    },
    render: function render() {
      var paper = this.options.paper;
      this.$activeBox = $('<div>').addClass('tree-layout-box active hidden').appendTo(this.el);
      ...
      this.svgPreviewChild = core_mjs.V(this.renderChildPreview()).attr(this.options.previewAttrs.child || {}).addClass('tree-layout-preview child');
      this.svgPreviewConnection = core_mjs.V(this.renderConnectionPreview()).attr(this.options.previewAttrs.link || {}).addClass('tree-layout-preview link');
      this.svgPreviewParent = core_mjs.V(this.renderParentPreview()).attr(this.options.previewAttrs.parent || {}).addClass('tree-layout-preview parent');
      this.svgPreview = core_mjs.V('g').addClass('tree-layout-preview-group').append([this.svgPreviewConnection, this.svgPreviewParent, this.svgPreviewChild]);
      this.$el.appendTo(paper.el);
      return this;
    },
    renderChildPreview: function renderChildPreview() {
      return core_mjs.V('circle');
    },
    renderParentPreview: function renderParentPreview() {
      return core_mjs.V('rect');
    },
    renderConnectionPreview: function renderConnectionPreview() {
    ...
    toggleDropping: function toggleDropping(state) {
    ...
    updateBox: function updateBox($box, bbox) {
      $box.css({
        width: bbox.width,
        height: bbox.height,
        left: bbox.x,
    ...
    positionTranslateBox: function positionTranslateBox(position) {
    ...
    prepareDraggingPaper: function prepareDraggingPaper(draggedElement) {
    ...
    dragstart: function dragstart(elements, x, y) {
      var $translateBox = this.$translateBox,
      ...
      if (elementView) {
        ...
        this.updateBox($translateBox, core_mjs.util.defaults({
          x: x,
          y: y
        }, bbox));
        ...
        var _bbox = core_mjs.V.transformRect(element.getBBox(), this.ctm);
        this.updateBox($translateBox, core_mjs.util.defaults({
          x: x,
          y: y
        }, _bbox));
    ...
    drag: function drag(elements, x, y) {
      var _this = this;
      ...
      if (layoutArea) {
        ...
        var validationFn = this.options.validatePosition;
        if (typeof validationFn === 'function') {
          isPositionValid = core_mjs.util.toArray(elements).every(function (element) {
            return validationFn.call(_this, element, x, y, _this);
          });
        }
        this.toggleDropping(isPositionValid);
      }
    },
    dragend: function dragend(elements, x, y) {
    ...
    show: function show() {
      var $mask = this.$mask,
        $activeBox = this.$activeBox,
        $translateBox = this.$translateBox,
        options = this.options;
      var $paper = options.paper.$el;
    ...
    reconnectElement: function reconnectElement(element, candidate) {
    ...
    reconnectElements: function reconnectElements(elements, candidate) {
      elements.forEach(function (element) {
        this.reconnectElement(element, candidate);
      }, this);
      this.layout();
    },
    translateElement: function translateElement(element, x, y) {
      var layout = this.model;
    ...
    translateElements: function translateElements(elements, x, y) {
      elements.forEach(function (element) {
    ...
    updatePreview: function updatePreview(siblings, siblingRank) {
    ...
    updateParentPreview: function updateParentPreview(position, size) {
    ...
    updateChildPreview: function updateChildPreview(position, size) {
    ...
    updateConnectionPreview: function updateConnectionPreview(source, target, vertices) {
      this.svgPreviewConnection.attr({
        d: core_mjs.connectors.rounded(source, target, vertices, {})
      });
    },
    findDirection: function findDirection(layoutArea, point) {
      var directions;
    ...
    // @private
    isConnectionValid: function isConnectionValid(element, siblings, siblingRank) {
      var parent = siblings.parentArea.root;
      ...
      if (typeof validationFn === 'function') {
        return validationFn.call(this, element, parent, this, {
          ...
          siblings: siblings.layoutAreas.map(function (la) {
            return la.root;
          })
        });
      }
    ...
    // Interaction
    canInteract: function canInteract(handler) {
    ...
    startDragging: function startDragging(elements) {
    ...
    onPointerdown: function onPointerdown(elementView) {
