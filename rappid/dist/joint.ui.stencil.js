...
(function (exports, $, core_mjs) {
	...
	*/
	...
	var FORCED$1 = 1 / $parseFloat(whitespaces + '-0') !== -Infinity
	  // MS Edge 18- broken with boxed symbols
	  || (ITERATOR$4 && !fails(function () { $parseFloat(Object(ITERATOR$4)); }));
	...
	var GridLayout = {
	  layout: function layout(graphOrCells, opt) {
	    var graph;
	    if (graphOrCells instanceof core_mjs.dia.Graph) {
	      ...
	      graph = new core_mjs.dia.Graph().resetCells(graphOrCells, {
	        dry: true,
	        sort: false
	      });
	    }
	    ...
	    var columnsX = this._accumulate(columnWidths, marginX).map(function (x, index) {
	      return x + (index - .5) * columnGap;
	    });
	    ...
	    var rowsY = this._accumulate(rowHeights, marginY).map(function (y, index) {
	      return y + (index - .5) * rowGap;
	    });
	    ...
	    var bbox = new core_mjs.g.Rect(marginX, marginY, columnsX[columns] - columnsX[0], rowsY[rows] - rowsY[0]);
	    return {
	  ...
	  _maxDim: function _maxDim(elements, dimension) {
	    return elements.reduce(function (max, el) {
	      return Math.max(el.get('size')[dimension], max);
	    }, 0);
	  },
	  _elementsAtRow: function _elementsAtRow(elements, rowIndex, numberOfColumns) {
	    var elementsAtRow = [];
	  ...
	  _elementsAtColumn: function _elementsAtColumn(elements, columnIndex, numberOfColumns) {
	  ...
	  _accumulate: function _accumulate(array, baseVal) {
	...
	var layoutDefaults = {
	   ...
	   */
	  layoutGroup: function layoutGroup(graph, group) {
	    var opts = this.options.layout;
	    group = group || {};
	    if (!GridLayout) {
	      throw new Error('joint.ui.Stencil: joint.layout.GridLayout is not available.');
	    }
	    GridLayout.layout(graph, core_mjs.util.assign({}, opts, group.layout));
	  }
	};
	var Stencil = core_mjs.mvc.View.extend({
	  ...
	  options: {
	     ...
	     * @returns {dia.Cell}
	     */
	    dragStartClone: function dragStartClone(cell) {
	      return cell.clone();
	    },
	    /**
	     * @param {dia.Cell} cell
	     * @returns {dia.Cell}
	     */
	    dragEndClone: function dragEndClone(cell) {
	      return cell.clone();
	    },
	   ...
	   */
	  setPaper: function setPaper(paper) {
	    var options = this.options;
	  ...
	  freeze: function freeze(opt) {
	    ...
	    Object.keys(papers).forEach(function (name) {
	   ...
	   * @returns {jQuery}
	   */
	  renderPaperDrag: function renderPaperDrag() {
	   ...
	   */
	  renderToggleAll: function renderToggleAll() {
	   ...
	   * @returns {jQuery}
	   */
	  renderElementsContainer: function renderElementsContainer() {
	  ...
	  renderGroup: function renderGroup(opt) {
	    opt = opt || {};
	    var $group = $('<div/>').addClass('group').attr('data-name', opt.name).toggleClass('closed', !!opt.closed);
	    var $label = $('<h3/>').addClass('group-label').html(opt.label || opt.name);
	  ...
	  render: function render() {
	    ...
	    this._paperDrag = new core_mjs.dia.Paper(core_mjs.util.assign({}, paperDragOptions, {
	      el: this.$paperDrag,
	      width: 1,
	      height: 1,
	      model: graphDrag,
	      sorting: core_mjs.dia.Paper.sorting.NONE
	    }));
	    this.startListening();
	    return this;
	  },
	  ...
	  startListening: function startListening() {
	    var _this = this;
	    this.stopListening();
	    var paperEvents = this.paperEvents,
	      papers = this.papers;
	    var _loop = function _loop(eventName) {
	      var method = paperEvents[eventName];
	      if (typeof method !== 'function') method = _this[method];
	      if (!method) return "continue";
	      core_mjs.util.forIn(papers, function (paper) {
	        // e.g. `cell:pointerdown` on any of the Stencil papers triggers element dragging.
	        _this.listenTo(paper, eventName, method);
	      });
	    };
	    for (var eventName in paperEvents) {
	   ...
	   * @param {string=} group
	   */
	  load: function load(cells, group) {
	   ...
	   */
	  loadGroup: function loadGroup(cells, group) {
	    var options = this.options;
	    ...
	    var paperHeight = options.height;
	    if (group) {
	   ...
	   * @returns {boolean}
	   */
	  isLayoutEnabled: function isLayoutEnabled() {
	   ...
	   * @returns {dia.Graph}
	   */
	  getGraph: function getGraph(group) {
	   ...
	   * @returns {dia.Paper}
	   */
	  getPaper: function getPaper(group) {
	    return this.papers[group || this.DEFAULT_GROUP];
	  },
	  hasSnaplinesEnabled: function hasSnaplinesEnabled() {
	    var snaplines = this.options.snaplines;
	    return Boolean(snaplines) && !snaplines.isDisabled();
	  },
	  preparePaperForDragging: function preparePaperForDragging(cellView, clientX, clientY) {
	    var paperDrag = this._paperDrag,
	      graphDrag = this._graphDrag,
	      options = this.options;
	    var snaplines = options.snaplines,
	      usePaperGrid = options.usePaperGrid,
	      scaleClones = options.scaleClones,
	      paper = options.paper,
	    ...
	    var clone = options.dragStartClone(cellView.model).position(0, 0);
	    ...
	    var cloneGeometryBBox = cloneView.getBBox({
	      useModelGeometry: true
	    });
	    var cloneViewDeltaOrigin = cloneGeometryBBox.origin().difference(cloneView.getBBox().origin());
	    var paperDragOffset = this.setPaperDragOffset(clientX, clientY, {
	      cloneGeometryBBox: cloneGeometryBBox,
	      cloneViewDeltaOrigin: cloneViewDeltaOrigin,
	  ...
	  removePaperAfterDragging: function removePaperAfterDragging() {
	    var paperDrag = this._paperDrag,
	      graphDrag = this._graphDrag;
	  ...
	  setPaperDragOffset: function setPaperDragOffset(clientX, clientY, _ref) {
	    var cloneViewDeltaOrigin = _ref.cloneViewDeltaOrigin,
	      cloneGeometryBBox = _ref.cloneGeometryBBox,
	      paperDragPadding = _ref.paperDragPadding;
	    ...
	    var left = clientX - cloneViewDeltaOrigin.x - cloneGeometryBBox.width / 2 - paperDragPadding;
	    var top = clientY - cloneViewDeltaOrigin.y - cloneGeometryBBox.height / 2 - paperDragPadding + scrollTop;
	  ...
	  onCloneSnapped: function onCloneSnapped(clone, position, opt) {
	    var _dragging = this._dragging;
	    if (!_dragging) return;
	    // Snapline plugin adds `snapped` flag when changing element's position
	    var snapped = opt.snapped,
	      tx = opt.tx,
	      ty = opt.ty;
	    if (snapped) {
	      var cloneBBox = _dragging.cloneBBox,
	        cloneView = _dragging.cloneView;
	      ...
	      clone.position(cloneBBox.x + tx, cloneBBox.y + ty, {
	        silent: true
	      });
	      cloneView.translate();
	  ...
	  _dragging: null,
	  onDragStart: function onDragStart(cellView, evt) {
	    var options = this.options,
	      _graphDrag = this._graphDrag;
	    var canDrag = options.canDrag,
	      snaplines = options.snaplines,
	      usePaperGrid = options.usePaperGrid;
	    var _cellView$model$graph = cellView.model.graph.attributes.group,
	      group = _cellView$model$graph === void 0 ? null : _cellView$model$graph; // `null` for stencils without groups
	    ...
	    var clientX = evt.clientX,
	      clientY = evt.clientY;
	    var _dragging = this.preparePaperForDragging(cellView, clientX, clientY);
	    var cloneView = _dragging.cloneView,
	      clone = _dragging.clone;
	    this._dragging = _dragging;
	    var validDropTarget = false;
	    var cloneArea = this.getCloneArea(cloneView, evt, validDropTarget );
	    core_mjs.util.assign(_dragging, {
	      clientX: clientX,
	      clientY: clientY,
	      validDropTarget: validDropTarget,
	      cloneArea: cloneArea
	    });
	    clone.position(cloneArea.x, cloneArea.y, {
	      silent: true
	    });
	  ...
	  onDrag: function onDrag(evt) {
	    ...
	    if (!_dragging) return;
	    evt.preventDefault();
	    var normalizedEvt = core_mjs.util.normalizeEvent(evt);
	    if (this.isDragCanceled()) {
	      // Drag was canceled by calling cancelDrag() not from the `drag` event callback.
	      var _cloneView = _dragging.cloneView,
	        _validDropTarget = _dragging.validDropTarget,
	        _cloneArea = _dragging.cloneArea;
	      this.notifyDragEnd(_cloneView, normalizedEvt, _cloneArea, _validDropTarget);
	      this._dragging = null;
	      return;
	    }
	    var cloneView = _dragging.cloneView;
	    var clone = cloneView.model;
	    var paper = options.paper,
	      snaplines = options.snaplines,
	      usePaperGrid = options.usePaperGrid;
	    var validDropTarget = this.insideValidArea({
	      x: normalizedEvt.clientX,
	      y: normalizedEvt.clientY
	    });
	    var cloneArea = this.getCloneArea(cloneView, normalizedEvt, validDropTarget && usePaperGrid);
	    var _cloneArea$center = cloneArea.center(),
	      x = _cloneArea$center.x,
	      y = _cloneArea$center.y;
	    var _paper$localToClientP = paper.localToClientPoint(x, y),
	      clientX = _paper$localToClientP.x,
	      clientY = _paper$localToClientP.y;
	    core_mjs.util.assign(_dragging, {
	      clientX: clientX,
	      clientY: clientY,
	      validDropTarget: validDropTarget,
	    ...
	    var embeddingMode = paper.options.embeddingMode;
	    if (embeddingMode) {
	      cloneView.eventData(normalizedEvt, {
	        paper: paper
	      });
	      var data = _dragging.cloneViewEventData = cloneView.eventData(normalizedEvt);
	      if (validDropTarget) {
	  ...
	  onDragEnd: function onDragEnd(evt) {
	    ...
	    if (!_dragging) return;
	    var normalizedEvt = core_mjs.util.normalizeEvent(evt);
	    var dragClone = _dragging.clone,
	      clientX = _dragging.clientX,
	      clientY = _dragging.clientY,
	      validDropTarget = _dragging.validDropTarget,
	      cloneArea = _dragging.cloneArea,
	      cloneView = _dragging.cloneView,
	      cloneBBox = _dragging.cloneBBox,
	      snapOffset = _dragging.snapOffset;
	    var x = cloneBBox.x;
	    var y = cloneBBox.y;
	    ...
	    var dropPoint = options.paper.clientToLocalPoint(clientX, clientY);
	    var snapToGrid = !snapOffset;
	    if (validDropTarget) {
	  ...
	  notifyDragEnd: function notifyDragEnd(cloneView, evt, cloneArea, validDropTarget) {
	  ...
	  onDrop: function onDrop(cloneView, evt, point, snapToGrid) {
	    var options = this.options;
	    var paper = options.paper,
	      graph = options.graph;
	    var embeddingMode = paper.options.embeddingMode;
	    var dragClone = cloneView.model;

	    // Start the dragging batch
	    // Batch might contain `add`, `change:parent`, `change:embeds` events.
	    graph.startBatch('stencil-drag');
	    var endClone = options.dragEndClone(dragClone);
	    ...
	    if (embeddingMode) {
	      cloneView.eventData(evt, {
	        model: endClone,
	        paper: paper,
	        initialParentId: core_mjs.util.uuid(),
	        // dummy parent id
	  ...
	  onDropInvalid: function onDropInvalid(cloneView, evt, point, _snapToGrid) {
	    var dragClone = cloneView.model;
	    var endClone = this.options.dragEndClone(dragClone);
	  ...
	  cancelDrag: function cancelDrag() {
	    var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.options,
	      dropAnimation = _ref2.dropAnimation;
	    var _dragging = this._dragging,
	      options = this.options,
	      _paperDrag = this._paperDrag;
	    if (!_dragging) return;
	    var clone = _dragging.clone,
	      cloneView = _dragging.cloneView,
	      cloneViewEventData = _dragging.cloneViewEventData,
	      paperDragInitialOffset = _dragging.paperDragInitialOffset;
	    var paper = options.paper,
	      snaplines = options.snaplines;
	    ...
	    if (dropAnimation) {
	      var _dropAnimation$durati = dropAnimation.duration,
	        duration = _dropAnimation$durati === void 0 ? 150 : _dropAnimation$durati,
	        _dropAnimation$easing = dropAnimation.easing,
	        easing = _dropAnimation$easing === void 0 ? 'swing' : _dropAnimation$easing;

	      // clear the view immediately to prevent user's interaction on the returning shape
	      _paperDrag.$el.animate(paperDragInitialOffset, duration, easing, this.removePaperAfterDragging.bind(this, clone));
	    } else {
	      this.removePaperAfterDragging(clone);
	    }
	  },
	  isDragCanceled: function isDragCanceled() {
	    var _dragging = this._dragging;
	    if (!_dragging) return false;
	    return Boolean(_dragging.canceled);
	  },
	  drop: function drop(endClone, point, snapToGrid) {
	    var options = this.options;
	    var paper = options.paper,
	      graph = options.graph;
	    var bbox = endClone.getBBox();
	    var position = new core_mjs.g.Point({
	  ...
	  insideValidArea: function insideValidArea(point) {
	    var _this$options = this.options,
	      paper = _this$options.paper,
	      paperScroller = _this$options.paperScroller;
	  ...
	  getDropArea: function getDropArea(el) {
	    var $el = $(el);
	  ...
	  getCloneArea: function getCloneArea(cloneView, evt, usePaperGrid) {
	    var paper = this.options.paper;
	    var clone = cloneView.model;
	    var clientX = evt.clientX,
	      clientY = evt.clientY;
	    var _paper$clientToLocalP = paper.clientToLocalPoint(clientX, clientY),
	      localCenterX = _paper$clientToLocalP.x,
	      localCenterY = _paper$clientToLocalP.y;
	    var _clone$size = clone.size(),
	      width = _clone$size.width,
	      height = _clone$size.height;
	    var x = localCenterX - width / 2;
	    var y = localCenterY - height / 2;
	    if (usePaperGrid) {
	      var _paper$snapToGrid = paper.snapToGrid(paper.localToClientPoint(x, y)),
	        snapX = _paper$snapToGrid.x,
	        snapY = _paper$snapToGrid.y;
	      x = snapX;
	      y = snapY;
	    }
	  ...
	  filter: function filter(keyword) {
	    var _this2 = this;
	    var cellAttributesMap = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.options.search;
	    ...
	    var match = Object.keys(this.papers).reduce(function (wasMatch, group) {
	      var paper = _this2.papers[group];
	      var groupId = group === _this2.DEFAULT_GROUP ? null : group;
	      ...
	      var matchedCells = paper.model.getCells().filter(function (cell) {
	        ...
	        if (core_mjs.util.isFunction(cellAttributesMap)) {
	          ...
	          cellMatch = cellAttributesMap.call(_this2, cell, keyword, groupId, _this2);
	        } else {
	          ...
	          cellMatch = _this2.isCellMatched(cell, keyword, cellAttributesMap, keyword.toLowerCase() !== keyword);
	        }
	      ...
	      _this2.trigger('filter', filteredGraph, group, keyword);
	      if (_this2.isLayoutEnabled()) {
	        _this2.layoutGroup(filteredGraph, _this2.getGroup(group));
	      }
	      if (_this2.$groups[group]) {
	        // add 'unmatched' class when filter matches no elements in the group
	        _this2.$groups[group].toggleClass('unmatched', !isMatch);
	      }
	      _this2.fitPaperToContent(paper);
	  ...
	  isCellMatched: function isCellMatched(cell, keyword, cellAttributesMap, caseSensitive) {
	  ...
	  fitPaperToContent: function fitPaperToContent(paper) {
	    var options = this.options;
	    var _paper$getComputedSiz = paper.getComputedSize(),
	      minWidth = _paper$getComputedSiz.width;
	    var contentOptions = core_mjs.util.assign({
	      minWidth: minWidth,
	   ...
	   * @private
	   */
	  pointerFocusIn: function pointerFocusIn() {
	    this.$el.addClass('is-focused');
	  },
	  /**
	   * @private
	   */
	  pointerFocusOut: function pointerFocusOut() {
	   ...
	   * @param {string} name
	   */
	  closeGroup: function closeGroup(name) {
	   ...
	   * @param {string} name
	   */
	  openGroup: function openGroup(name) {
	    var $group = this.$groups[name];
	    if (!$group || this.isGroupOpen(name)) return;
	   ...
	   * @param {string} name
	   */
	  isGroupOpen: function isGroupOpen(name) {
	    var $group = this.$groups[name];
