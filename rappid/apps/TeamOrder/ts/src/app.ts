...
const addTeam = (members: MemberType[], graph: dia.Graph, paper: dia.Paper): void => {
...
const shuffleTeam = (stackLayoutView: ui.StackLayoutView): void => {
    ...
    const members = stackModel.stacks[COLUMNS - 1].elements;

    for (let i = members.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [members[i], members[j]] = [members[j], members[i]];
        members[i].set('stackElementIndex', i);
        members[j].set('stackElementIndex', j);
    }
