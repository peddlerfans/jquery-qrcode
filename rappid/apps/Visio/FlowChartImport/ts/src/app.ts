...
export const init = async() => {
    ...
    const headerSize = 30;
    const { x, y, width, height } = graph.getBBox().inflate(headerSize + 30, 50);
    const lanes = Array.from(lanesMap.values()).map((record, index, lanes) => {
        ...
        if (nextRecord) {
            ...
            const y0 = (y1 + y2) / 2;
            nextRecord.y = y0;
            poolLane.size = y0 - ((index > 0) ? record.y : y);
        }
        return poolLane;
    });
