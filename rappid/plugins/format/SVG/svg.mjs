...
export const toSVG = function(paper, callback, opt) {
    ...
    if (opt.convertImagesToDataUris) {
        ...
        convertImages(exportSVG.find('image')).then(() => callbackWrapper());
...
function convertImages(images) {
    ...
    images.forEach((image) => {
        ...
        let url = image.attr('xlink:href') || image.attr('href');
...
function convertImageURL(url, images) {
    ...
    util.imageToDataUri(url, (err, dataUri) => {
        ...
        images.forEach(image => image.attr('xlink:href', dataUri));
        deferred.resolve();
    });
