...
export const Pool = dia.Element.define('bpmn2.Pool', {
    ...
    attrs: {
        ...
        laneLabels: {
            ...
            labelMargin: { vertical: 2, horizontal: 10 },
    ...
    buildMilestones: function(milestones, markup) {
        ...
        milestones.forEach((milestone, index) => {
            ...
            if (name) {
                const nameOverwritesPreviousMilestoneId = (name !== milestoneGroupId) && metrics.milestones[name] !== undefined;
                const nameAlreadyExists = metrics.milestonesNameCache[name] !== undefined;

                if (nameAlreadyExists || nameOverwritesPreviousMilestoneId) {
                    error('Duplicated milestone group id: ' + name);
                }
                metrics.milestonesNameCache[name] = milestoneGroupId;
            }
            if (otherCustomIdOverwritesThisId) {
                error('Duplicated milestone group id: ' + milestoneGroupId);
            }

            const gContainerMarkup = this.getMilestoneGroupMarkup(index, name);
            markup.push(gContainerMarkup);

            const header = this.getMilestoneHeaderMarkup(index, name);
            const label = this.getMilestoneLabelMarkup(index, name);
            const line = this.getMilestoneLineMarkup(index, name);
    ...
    addLaneGroupMetrics: function(uniqueId, laneGroupMetrics) {
        ...
        if (name) {
            const nameOverwritesPreviousLaneId = (name !== laneGroupId) && metrics.lanes[name] !== undefined;
            const nameAlreadyExists = metrics.lanesNameCache[name] !== undefined;

            if (nameAlreadyExists || nameOverwritesPreviousLaneId) {
                error('Duplicated lane group id: ' + name);
            }
            metrics.lanesNameCache[name] = laneGroupId;
        }
    ...
    getLanesFromPoint: function(point) {
        ...
        const recursion = (parentId) => {
            ...
            while (tempIndex >= 0) {
                ...
                if (bbox.containsPoint(pointUnrotated)) {
                    const customId = metricsLanes[laneId].name;
    ...
    getMilestoneFromPoint: function(point) {
        ...
        while (tempIndex >= 0) {
            ...
            if (milestoneBBox.containsPoint(pointUnrotated)) {
                const customId = metrics.milestones[milestoneId].name;
